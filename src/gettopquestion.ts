...
async function getQuestions(): Promise<void> {
    try {
        ...
        const result: any = await api.queryDatabase(
            "SELECT questions.questionId, questions.title, questions.description, questions.created_at, questions.code, user2.firstname, user2.lastname, SUM(ratings.ratingValue) as totalRating FROM questions LEFT JOIN user2 ON questions.userId=user2.id LEFT JOIN ratings ON questions.questionId=ratings.questionId GROUP BY questions.questionId ORDER BY totalRating DESC"
        );
